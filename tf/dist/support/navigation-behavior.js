      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            _process$env$UI_AUTOM = process.env.UI_AUTOMATION_HOST, hostname = _process$env$UI_AUTOM === void 0 ? 'development' : _process$env$UI_AUTOM;
            hostpath = hostsConfig["".concat(hostname)];

            _logger.logger.log('🌐 URL:', hostpath);

            url = new URL(hostpath);
            pagesConfigItem = pagesConfig[pageId];
            url.pathname = pagesConfigItem.route;

            _logger.logger.log('🌐 Page path:', url.pathname);
